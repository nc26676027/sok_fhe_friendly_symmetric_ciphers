...
func (eval *ckksEvaluator) RotateHoisted(ct0 *Ciphertext, rotations []int) (cOut map[int]*Ciphertext) {
	...
	for _, i := range rotations {
		...
		if i == 0 {
			...
			cOut[i] = NewCiphertextCKKS(eval.params, 1, level, ct0.Scale())
...
func (eval *mfvEvaluator) LinearTransform(vec *Ciphertext, linearTransform interface{}) (res []*Ciphertext) {
	level := vec.Level()
	vecNTT := &Ciphertext{vec.CopyNew()}
	eval.ringQ.NTTLvl(level, vec.value[0], vecNTT.value[0])
	eval.ringQ.NTTLvl(level, vec.value[1], vecNTT.value[1])

	switch element := linearTransform.(type) {
	case []*PtDiagMatrixT:
		res = make([]*Ciphertext, len(element))

		eval.DecompInternal(vec.value[1], eval.c2QiQDecomp, eval.c2QiPDecomp)

		for i, matrix := range element {
			res[i] = NewCiphertextFVLvl(eval.params, 1, level)
			eval.MultiplyByDiabMatrix(vecNTT, res[i], matrix, eval.c2QiQDecomp, eval.c2QiPDecomp)
		}
	case *PtDiagMatrixT:
		eval.DecompInternal(vec.value[1], eval.c2QiQDecomp, eval.c2QiPDecomp)

		res = []*Ciphertext{NewCiphertextFVLvl(eval.params, 1, level)}

		eval.MultiplyByDiabMatrix(vecNTT, res[0], element, eval.c2QiQDecomp, eval.c2QiPDecomp)
	}
	return
}

func (eval *ckksEvaluator) LinearTransform(vec *Ciphertext, linearTransform interface{}) (res []*Ciphertext) {
	...
	case []*PtDiagMatrix:
		...
		for i, matrix := range element {
			res[i] = NewCiphertextCKKS(eval.params, 1, minLevel, vec.Scale())
	...
	case *PtDiagMatrix:
		...
		res = []*Ciphertext{NewCiphertextCKKS(eval.params, 1, minLevel, vec.Scale())}
...
func (eval *ckksEvaluator) InnerSum(ct0 *Ciphertext, batchSize, n int, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) InnerSumNaive(ct0 *Ciphertext, batchSize, n int, ctOut *Ciphertext) {
...
func (eval *mfvEvaluator) MultiplyByDiabMatrix(vecNTT, res *Ciphertext, matrix *PtDiagMatrixT, c2QiQDecomp, c2QiPDecomp []*ring.Poly) {
	if matrix.naive {
		eval.MultiplyByDiabMatrixNaive(vecNTT, res, matrix, c2QiQDecomp, c2QiPDecomp)
	} else {
		eval.MultiplyByDiabMatrixBSGS(vecNTT, res, matrix, c2QiQDecomp, c2QiPDecomp)
	}
}

func (eval *ckksEvaluator) MultiplyByDiabMatrix(vec, res *Ciphertext, matrix *PtDiagMatrix, c2QiQDecomp, c2QiPDecomp []*ring.Poly) {
...
func (eval *mfvEvaluator) MultiplyByDiabMatrixNaive(vecNTT, res *Ciphertext, matrix *PtDiagMatrixT, c2QiQDecomp, c2QiPDecomp []*ring.Poly) {
	ringQ := eval.ringQ
	ringP := eval.ringP
	...
	for k := range matrix.Vec {
		...
		if k == 0 {
			...
			if !generated {
				panic("switching key not avaliable")
			}
	...
	eval.baseconverterQ1P.ModDownSplitNTTPQ(levelQ, res.value[0], accP0, res.value[0]) // sum(phi(c0 * P + d0_QP))/P
	eval.baseconverterQ1P.ModDownSplitNTTPQ(levelQ, res.value[1], accP1, res.value[1]) // sum(phi(d1_QP))/P

	if state { // Rotation by zero
		ringQ.MulCoeffsMontgomeryAndAddLvl(levelQ, matrix.Vec[0][0], vecNTT.value[0], res.value[0]) // res += c0_Q * plaintext
		ringQ.MulCoeffsMontgomeryAndAddLvl(levelQ, matrix.Vec[0][0], vecNTT.value[1], res.value[1]) // res += c1_Q * plaintext
	}

	ringQ.InvNTTLvl(levelQ, res.value[0], res.value[0])
	ringQ.InvNTTLvl(levelQ, res.value[1], res.value[1])
}

func (eval *ckksEvaluator) MultiplyByDiabMatrixNaive(vec, res *Ciphertext, matrix *PtDiagMatrix, c2QiQDecomp, c2QiPDecomp []*ring.Poly) {
...
func (eval *mfvEvaluator) MultiplyByDiabMatrixBSGS(vecNTT, res *Ciphertext, matrix *PtDiagMatrixT, c2QiQDecomp, c2QiPDecomp []*ring.Poly) {
	...
	index, rotations := bsgsIndex(matrix.Vec, 1<<matrix.LogFVSlots, matrix.N1)
	...
	ringQ.MulScalarBigintLvl(levelQ, vecNTT.value[0], ringP.ModulusBigint, tmpQ0) // P*c0
	...
	for j := range index {
		...
		if j != 0 {
			...
			eval.baseconverterQ1P.ModDownSplitNTTPQ(levelQ, tmpQ0, pool2P, tmpQ0) // sum(phi(d0) * plaintext)/P
			eval.baseconverterQ1P.ModDownSplitNTTPQ(levelQ, tmpQ1, pool3P, tmpQ1) // sum(phi(d1) * plaintext)/P
			...
			if state {
				...
				if len(index[j]) == 1 {
					ringQ.MulCoeffsMontgomeryLvl(levelQ, matrix.Vec[N1*j][0], vecNTT.value[0], tmpQ0) // c0 * plaintext + sum(phi(d0) * plaintext)/P + phi(c0) * plaintext mod Q
					ringQ.MulCoeffsMontgomeryLvl(levelQ, matrix.Vec[N1*j][0], vecNTT.value[1], tmpQ1) // c1 * plaintext + sum(phi(d1) * plaintext)/P + phi(c1) * plaintext mod Q
				} else {
					ringQ.MulCoeffsMontgomeryAndAddLvl(levelQ, matrix.Vec[N1*j][0], vecNTT.value[0], tmpQ0) // c0 * plaintext + sum(phi(d0) * plaintext)/P + phi(c0) * plaintext mod Q
					ringQ.MulCoeffsMontgomeryAndAddLvl(levelQ, matrix.Vec[N1*j][0], vecNTT.value[1], tmpQ1) // c1 * plaintext + sum(phi(d1) * plaintext)/P + phi(c1) * plaintext mod Q
				}
	...
	eval.baseconverterQ1P.ModDownSplitNTTPQ(levelQ, tmpQ2, tmpP2, tmpQ2) // sum(phi(c0 * P + d0_QP))/P
	eval.baseconverterQ1P.ModDownSplitNTTPQ(levelQ, tmpQ3, tmpP3, tmpQ3) // sum(phi(d1_QP))/P
...
func (eval *ckksEvaluator) MultiplyByDiabMatrixBSGS(vec, res *Ciphertext, matrix *PtDiagMatrix, c2QiQDecomp, c2QiPDecomp []*ring.Poly) {
