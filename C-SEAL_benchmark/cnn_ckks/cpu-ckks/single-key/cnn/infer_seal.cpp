...
//****************    transcipher    *******************
	...
	for(size_t i=0;i<4;i++){
		...
		Ciphertext rtncipher = transcipher.construct_number_from_bits(bits, 16);
	...
	auto convert_time_end = chrono::high_resolution_clock::now();
	auto convert_time_duration = chrono::duration_cast<chrono::milliseconds>(convert_time_end - time_start_convert);
	cout << "convert time: " << convert_time_duration.count() / 1000 << " ms" << endl;	
...
cout<<"Not running transciphering!\n\n\n"<<endl;
	...
	// for(size_t image_id = 0; image_id <=0; image_id++)
	{
		...
		// modulus down
		Ciphertext ctxt = cnn_cipher.cipher();
		bootstrapper_1.slim_bootstrap(ctxt, ctxt);
		cnn_cipher.set_ciphertext(ctxt);
		cur_level = context.get_context_data(cnn_cipher.cipher().parms_id())->chain_index();
		cout << "Before resnet remaining level : " << cur_level << endl;
		cout << "scale: " << cnn_cipher.cipher().scale() << endl;
		// layer 0
		cout << "layer 0" << endl;
		output << "layer 0" << endl;
		...
		if(plain_dnn){
			...
			cnn_cipher.set_ciphertext(ctxt);
			cur_level = context.get_context_data(cnn_cipher.cipher().parms_id())->chain_index();
			cout << "Before first conv remaining level : " << cur_level << endl;
			cout << "scale: " << cnn_cipher.cipher().scale() << endl;
			...
			ReLU_seal_part( cnn_cipher, cnn_cipher, comp_no, deg, alpha, tree, scaled_val, logp, encryptor, evaluator, decryptor, encoder, public_key, secret_key, relin_keys, bootstrapper_1, B);
		}
			...
			for(int k=0; k<=end_num; k++)	// 0 ~ 2/4/6/8/17
			{
				...
				if(plain_dnn){
					...
					if(j==0){
						...
						bootstrapper_2.slim_bootstrap(ctxt, ctxt);
						cnn_cipher.set_ciphertext(ctxt);
						ReLU_seal_part( cnn_cipher, cnn_cipher, comp_no, deg, alpha, tree, scaled_val, logp, encryptor, evaluator, decryptor, encoder, public_key, secret_key, relin_keys, bootstrapper_2, B);
					} 
					else if(j==2) {
						bootstrapper_3.slim_bootstrap(ctxt, ctxt);
						cnn_cipher.set_ciphertext(ctxt);
						ReLU_seal_part( cnn_cipher, cnn_cipher, comp_no, deg, alpha, tree, scaled_val, logp, encryptor, evaluator, decryptor, encoder, public_key, secret_key, relin_keys, bootstrapper_3, B);
					}
