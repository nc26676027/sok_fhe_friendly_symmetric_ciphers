...
//         }
        ...
        void dyadic_product_coeffmod( // Chao added
            ConstCoeffIter operand1, ConstCoeffIter operand2, size_t coeff_count, const Modulus &modulus,
            CoeffIter result)
        {
#ifdef SEAL_DEBUG
            ...
            SEAL_ITERATE(iter(operand1, operand2, result), coeff_count, [&](auto I) {
                ...
                unsigned long long input[2], quo;
                multiply_uint64(get<0>(I), get<1>(I), input);
                // Chao added Barret reduction
                ...
                uint64_t tmp = quo * modulus_value;
                ...
                // Claim: One more subtraction is enough
                get<2>(I) = SEAL_COND_SELECT(Z >= modulus_value, Z - modulus_value, Z);
            });
#endif
        }
