...
namespace seal
{
    ...
    void Modulus::set_value(uint64_t value)
    {
        ...
        else
        {
            ...
            // Compute Barrett ratios for 32-bit words (barrett_reduce_64)
            ...
            // Chao added 
            uint64_t numerator128[2]{ 1, 0 };
            uint64_t quotient128[2]{ 0, 0 };
            left_shift_uint128(numerator128, SEAL_MOD_BIT_COUNT_MAX+bit_count_+1, numerator128);
            // cout << "numerate128:  0x"<<hex<<numerator128[1]<<dec<<endl;
            // throw invalid_argument("first");
            // Use a special method to avoid using memory pool
            divide_uint128_inplace(numerator128, value_, quotient128);

            const_ratio128_[0] = quotient128[0];
            // We store also the remainder
            const_ratio128_[1] = numerator128[0];
