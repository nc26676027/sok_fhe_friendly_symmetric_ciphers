...
namespace seal
{
    ...
    void Evaluator::align_inplace_reduced_error(Ciphertext &encrypted1, const Ciphertext &encrypted2) const {
        size_t encrypted1_coeff_modulus_size = encrypted1.coeff_modulus_size();
        size_t encrypted2_coeff_modulus_size = encrypted2.coeff_modulus_size();
        ...
        else
        {
			...
			mod_switch_to_inplace(encrypted1_adjusted, encrypted2.parms_id());
			encrypted1_adjusted.scale() = encrypted2.scale();	
            encrypted1 = encrypted1_adjusted;       
            // multiply_const(encrypted1, scale_adjust, encrypted1_adjusted);
            // rescale_to_next_inplace(encrypted1_adjusted);
            // mod_switch_to_inplace(encrypted1_adjusted, encrypted2.parms_id());
            // encrypted1_adjusted.scale() = encrypted2.scale();	
        ...
        MemoryPoolHandle pool) const
    {
        ...
        size_t rns_modulus_size = decomp_modulus_size + 1; //dcomp_num = L+1 for SEAL 3.6 
   ...
   void Evaluator::switch_key_inplace(Ciphertext &encrypted, ConstRNSIter target_iter, const KSwitchKeys &kswitch_keys, size_t kswitch_keys_index, MemoryPoolHandle pool) const
    {
        ...
        auto &key_parms = key_context_data.parms();
        auto &key_modulus = key_parms.coeff_modulus();
        auto small_ntt_tables = key_context_data.small_ntt_tables(); //small NTT table
        auto scheme = parms.scheme();
 
        // Verify parameters.
        const size_t degree  = parms.poly_modulus_degree();
        const size_t n_ct_rns = parms.coeff_modulus().size();//ct rns number
        const size_t n_ct_all_rns = context_.get_context_data(context_.first_parms_id())->parms().coeff_modulus().size();// initial rns number = L
        const size_t n_total_rns = key_modulus.size(); // total rns = L+k
        const size_t n_special_rns = n_total_rns - n_ct_all_rns;
        const size_t n_bundles = (n_ct_rns + n_special_rns - 1) / n_special_rns; // (L + k - 1)/k = \alpha
        const bool is_ckks = scheme == scheme_type::ckks;

                //Chao test
        // cout <<"\n\ndegree: "<<degree<<", n_ct_rns: "<<n_ct_rns<<", n_ct_all_rns: "<<n_ct_all_rns<<endl;
        // cout <<"n_total_rns: "<<n_total_rns<<", n_special_rns: "<<n_special_rns<<", n_bundles: "<<n_bundles<<endl;
        ...
        for (size_t src_bundle_idx = 0; src_bundle_idx < n_bundles; ++src_bundle_idx) {
            ...
            for (size_t k = 0; k < n_ct_rns + n_special_rns; ++k) {
                ...
                if (k >= rns0 && k < rns1) {
                    ct_ptr = *(target_iter + k); // Iter version changed
                } else {
        ...
        for (size_t b : {0, 1}) {
            ...
            for (size_t k = 0; k < n_ct_rns + n_special_rns; ++k) {
                ...
                for (size_t l = 0; l < degree; l++, lazy_mult_ptr += 2) {
                    *dst_ptr++ = barrett_reduce_128_nonlazy(lazy_mult_ptr, key_modulus[rns_idx]);//Chao added 1
                }
