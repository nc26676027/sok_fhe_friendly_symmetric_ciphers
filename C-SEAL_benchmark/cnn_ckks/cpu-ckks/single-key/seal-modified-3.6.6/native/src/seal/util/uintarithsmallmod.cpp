    ...
    namespace util
    {
        uint64_t exponentiate_uint_mod(uint64_t operand, uint64_t exponent, const Modulus &modulus)
        {
            ...
            while (true)
            {
                if (exponent & 1)
                {
                    product = multiply_uint_mod_nonlazy(power, intermediate, modulus);//fourth
                ...
                product = multiply_uint_mod_nonlazy(power, power, modulus); // fifth
                swap(product, power);
            }
            return intermediate;
        }
            ...
            const uint64_t *operand1, const uint64_t *operand2, size_t count, const Modulus &modulus)
        {
            ...
            largest_case:
                ...
                accumulator[0] = barrett_reduce_128_nonlazy(accumulator, modulus);
                break;
            // case 2:
            ...
            //     break;
            default:
                accumulator[0] = dot_product_mod(operand1 + 1, operand2 + 1, count - 1, modulus);
                goto largest_case;
            };
