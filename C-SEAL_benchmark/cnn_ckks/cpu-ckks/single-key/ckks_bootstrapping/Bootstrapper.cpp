...
Bootstrapper::Bootstrapper(long _loge, long _logn, long _logNh, long _L, double _final_scale, long _boundary_K, long _sin_cos_deg, long _scale_factor, long _inverse_deg,
	...
	n = 1 << logn;
	Nh = 1 << logNh;
	mod_reducer = new ModularReducer(boundary_K, (double)loge, sin_cos_deg, scale_factor, inverse_deg, context, encoder, encryptor, evaluator, relin_keys, decryptor);
}
...
void Bootstrapper::addBootKeys(GaloisKeys &gal_keys) {
    ...
    if (slot_index == -1) throw("LT coefficients were not generated for this logn");
...
void Bootstrapper::genfftcoeff_full_one_depth() {
    ...
    for(int u = 0; u < slot_vec.size(); u++) {
        ...
        for(int i = 0; i < totlen1 + 1; i++) fftcoeff1[u][i].resize(curr_n, 0); 
...
void Bootstrapper::genfftcoeff() {
    ...
    for(int u = 0; u < slot_vec.size(); u++) {
        ...
        for(int i = 0; i < 2*totlen1 + 1; i++) fftcoeff1[u][i].resize(2 * curr_n, 0); 
        for(int i = 0; i < 2*totlen2 + 1; i++) fftcoeff2[u][i].resize(2 * curr_n, 0); 
        ...
        for (int i = 0; i < 2*totlen1 + 1; i++) {
            for (int j = 0; j < curr_n; j++) fftcoeff1[u][i][j + curr_n] = fftcoeff1[u][i][j]; 
        }
        ...
        for (int i = 0; i < 2*totlen2 + 1; i++) {
            for (int j = 0; j < curr_n; j++) fftcoeff2[u][i][j + curr_n] = complex<double>(0, 1) * fftcoeff2[u][i][j]; 
        }
    }
}
...
void Bootstrapper::genfftcoeff_full() {
    ...
    for(int u = 0; u < slot_vec.size(); u++) {
        ...
        for(int i = 0; i < 2*totlen1 + 1; i++) fftcoeff1[u][i].resize(curr_n, 0); 
        for(int i = 0; i < totlen2 + 1; i++) fftcoeff2[u][i].resize(curr_n, 0); 
...
void Bootstrapper::geninvfftcoeff() {
    ...
    for(int u = 0; u < slot_vec.size(); u++) {
        ...
        for(int i = 0; i < totlen1 + 1; i++) invfftcoeff1[u][i].resize(curr_n, 0); 
        for(int i = 0; i < 2*totlen2 + 1; i++) invfftcoeff2[u][i].resize(2 * curr_n, 0); 
        ...
        for (int i = 0; i < totlen1 + 1; i++) {
            for (int j = 0; j < curr_n; j++) invfftcoeff1[u][i][j] *= 1.0 / (boundary_K * (1 << (logNh - curr_logn)));
        }
...
void Bootstrapper::geninvfftcoeff_full() {
    ...
    vector<int> tmpcount;
    int totlen1, totlen2, split_point, basicstep, all_case_count, pos, current_pos, ind, ind_res, curr_logn, curr_n;
    // std::cout << "before slot_vec gen"<<endl;
    for(int u = 0; u < slot_vec.size(); u++) {
        curr_logn = slot_vec[u];
        curr_n = (1 << curr_logn);
        split_point = ceil(curr_logn / 2.0);
        ...
        for (int i = 0; i < 2*totlen2 + 1; i++) {
            for (int j = 0; j < curr_n; j++) invfftcoeff2[u][i][j] *= 0.5; 
        }
    }
}
...
void Bootstrapper::genfftcoeff_3() { // not yet
    ...
    for(int u = 0; u < slot_vec.size(); u++) {
        ...
        if (curr_logn == logNh) {
            ...
            for(int i = 0; i < 2*totlen1 + 1; i++) fftcoeff1[u][i].resize(2 * curr_n, 0); 
            for(int i = 0; i < 2*totlen2 + 1; i++) fftcoeff2[u][i].resize(2 * curr_n, 0); 
            for(int i = 0; i < 2*totlen3 + 1; i++) fftcoeff3[u][i].resize(2 * curr_n, 0); 
            ...
            for (int i = 0; i < 2*totlen2 + 1; i++) {
                for (int j = 0; j < curr_n; j++) fftcoeff2[u][i][j + curr_n] = fftcoeff2[u][i][j]; 
            }
            ...
            for (int i = 0; i < 2*totlen3 + 1; i++) {
                for (int j = 0; j < curr_n; j++) fftcoeff3[u][i][j + curr_n] = complex<double>(0, 1) * fftcoeff3[u][i][j]; 
            }
        }
    }
}
...
void Bootstrapper::geninvfftcoeff_3() { // not yet
    ...
    for(int u = 0; u < slot_vec.size(); u++) {
        ...
        if (curr_logn == logNh) {
            ...
            for(int i = 0; i < totlen1 + 1; i++) invfftcoeff1[u][i].resize(curr_n, 0); 
            for(int i = 0; i < 2*totlen2 + 1; i++) invfftcoeff2[u][i].resize(curr_n, 0); 
            for(int i = 0; i < 2*totlen3 + 1; i++) invfftcoeff3[u][i].resize(2 * curr_n, 0); 
...
void Bootstrapper::prepare_mod_polynomial() {
    ...
    // cout << "after generate sincos polynomial"<<endl;
    mod_reducer->generate_inverse_sine_polynomial();
    // cout << "after generate inverse sine polynomial"<<endl;
    mod_reducer->write_polynomials();
}
...
void Bootstrapper::slim_sfl_3(Ciphertext &rtncipher, Ciphertext &cipher) { 
    ...
    for (int i = 0; i < 2 * totlen3 + 1; i++) {
        for (int j = 0; j < 2 * n; j++) {
            fftcoeff3_scale[i][j] = fftcoeff3[slot_index][i][j] * ( cipher.scale() * 2 ) / cipher.scale();
        }
    }
...
void Bootstrapper::slim_sfl_full_3(Ciphertext &rtncipher, Ciphertext &cipher) { 
    ...
    // cout << "fft mod_zero: "<<log2(mod_zero)<<endl;
    // cout << "fft curr_mod: "<<log2(curr_mod)<<endl;
    // cout << "fft final_scale: "<<log2(final_scale)<<endl;
    // cout << "fft tmpct2.scale: "<<log2( tmpct2.scale() )<<endl;
    // cout << "fft initial_scale: "<<log2(initial_scale)<<endl;

    for (int i = 0; i < totlen2 + 1; i++) {
        for (int j = 0; j < n; j++) {
            // fftcoeff3_scale[i][j] = fftcoeff3[slot_index][i][j] * curr_mod * mod_zero * final_scale / (tmpct2.scale() * tmpct2.scale() * initial_scale);
            fftcoeff3_scale[i][j] = fftcoeff3[slot_index][i][j];
        }
    }
...
void Bootstrapper::modraise_inplace(Ciphertext &cipher) {
    ...
    minus_q0[0] = 0;
    //minus_q0 is number in each level to fulfill interger minu q0
    for (size_t l = 1; l < coeff_modulus_size; l++) {
        minus_q0[l] = modulus[l].value() - q0 % modulus[l].value();
        // cout << "minus_q0: "<<minus_q0[l]<<endl;
    }
...
//TODO
void Bootstrapper::slim_bootstrap_sparse(Ciphertext &rtncipher, Ciphertext &cipher) 
{
...
void Bootstrapper::bootstrap_full_3(Ciphertext &rtncipher, Ciphertext &cipher) {
	...
	// cout << "Modulus Raising..." << endl;
    cout<<"before modraise scale: "<<log2(cipher.scale())<<endl;
	modraise_inplace(cipher);
    ...
    cout<<"after C2S scale: "<<log2(cipher.scale())<<endl;
...
void Bootstrapper::slim_bootstrap(Ciphertext &rtncipher, Ciphertext &cipher) {
    cipher.scale() = pow(2.0, round( log2(cipher.scale()) ) );
    initial_scale = cipher.scale();
    if (logn == logNh) slim_bootstrap_full_real_3(rtncipher, cipher);
    else {
        // cout << "sparse"<<endl;
        slim_bootstrap_sparse(rtncipher, cipher);
    }
}
