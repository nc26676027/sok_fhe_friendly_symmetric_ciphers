...
void minimax_ReLU_seal_part(long comp_no, vector<int> deg, long alpha, vector<Tree> &tree, double scaled_val, long scalingfactor, Encryptor &encryptor, Evaluator &evaluator, Decryptor &decryptor, CKKSEncoder &encoder, PublicKey &public_key, SecretKey &secret_key, RelinKeys &relin_keys, Ciphertext& cipher_in, Ciphertext& cipher_res, Bootstrapper &btp)
{
	...
	// cout << "before evaluation"<<endl;
	// // evaluating pk ... p1(x) / 2
	// decrypt_and_print_part(cipher_x, decryptor, encoder, n, 6, 6);
	...
	btp.slim_bootstrap(cipher_x, cipher_x);
	eval_polynomial_integrate(encryptor, evaluator, decryptor, encoder, public_key, secret_key, relin_keys, cipher_x, cipher_x, deg[comp_no-1], decomp_coeff[comp_no-1], tree[comp_no-1]); 
	// cout << "after evaluation"<<endl;
	// x(1+sgn(x))/2 from sgn(x)/2
	encoder.encode(m_half, cipher_x.scale(), plain_half);
	encryptor.encrypt(plain_half, cipher_half);	
