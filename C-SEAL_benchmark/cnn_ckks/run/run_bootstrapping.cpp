...
int main() {
	...
	// modified SEAL
    parms.set_secret_key_hamming_weight(secret_key_hamming_weight);//?? whats the meaning of this parm
	parms.set_sparse_slots(sparse_slots);//?? whats the meaning of this parm, sparse_slots? packing fomart?
	...
	Bootstrapper bootstrapper_2(loge, logn_2, logN - 1, total_level, scale, boundary_K, deg, scale_factor, inverse_deg, context, keygen, encoder, encryptor, decryptor, evaluator, relin_keys, gal_keys);
	Bootstrapper bootstrapper_3(loge, logn_3, logN - 1, total_level, scale, boundary_K, deg, scale_factor, inverse_deg, context, keygen, encoder, encryptor, decryptor, evaluator, relin_keys, gal_keys);
	...
	vector<complex<double>> sparse(sparse_slots);
	vector<complex<double>> input(slot_count);
	vector<complex<double>> before(slot_count);
	vector<complex<double>> after(slot_count);

    random_complex(sparse, sparse_slots); //generate random complex number
	...
	{	
        if (_ == 0) sparse_slots = (1 << logn);
        else if (_ == 1) sparse_slots = (1 << logn_2);
        else if (_ == 2) sparse_slots = (1 << logn_3);

		cout << _ << "-th iteration : sparse_slots = " << sparse_slots << endl;
		 
		for (size_t i = 0; i < slot_count; i++) {
				input[i] = static_cast<double>(1 << log_integer_part) * sparse[i%sparse_slots];
		}
		...
		for (int i = 0; i < total_level; i++) {
			...
			cout << "remaining level : " << context.get_context_data(cipher.parms_id())->chain_index() << endl;
	cout << "scale: " << cipher.scale() << endl << endl;
		Ciphertext rtn;
		...
		if (_ == 0) bootstrapper.bootstrap_3(rtn, cipher);
        else if (_ == 1) bootstrapper_2.bootstrap_3(rtn, cipher);
		else if (_ == 2) bootstrapper_3.bootstrap_3(rtn, cipher);
		...
		for (long i = 0; i < sparse_slots; i++)
		{
			...
			mean_err += abs(before[i].real() - after[i].real());
			mean_err += abs(before[i].imag() - after[i].imag());
			// if (file.is_open())
			// {
