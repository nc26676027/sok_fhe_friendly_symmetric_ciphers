...
namespace boot
{
	...
	void Polynomial::showcoeff() {
		for(int i = 0; i < deg + 1; i++) {
			cout << "coeff term " << i << " : " << coeff[i] << endl;
			// cout << coeff[i] << endl;
		}
		cout << endl;
	}
	...
	void Polynomial::homomorphic_poly_evaluation(SEALContext &context, CKKSEncoder &encoder, Encryptor &encryptor, Evaluator &evaluator, RelinKeys &relin_keys, Ciphertext &rtn, Ciphertext &cipher, Decryptor &decryptor) {
		double zero = 1./ (cipher.scale()*cipher.scale());
		// generation of zero ciphertext 
		Ciphertext ctxt_zero;
		vector<double> m_zero(encoder.slot_count(), 0.0);
		Plaintext plain_zero;
		encoder.encode(m_zero, cipher.scale(), plain_zero);		// scaling factor: scale^2 for lazy scaling
	...
	void Polynomial::homomorphic_poly_evaluation_binary(SEALContext &context, CKKSEncoder &encoder, Encryptor &encryptor, Evaluator &evaluator, RelinKeys &relin_keys, Ciphertext &rtn, Ciphertext &cipher, Decryptor &decryptor) {
		double zero = 1./cipher.scale();
		// generation of zero ciphertext 
		Ciphertext ctxt_zero;
		vector<double> m_zero(encoder.slot_count(), 0.0);
