...
//	RRprec = 2000;
	...
	Remez rm(BinBoot, inter_num, inter_start, inter_end, sc, prec, deg, iteration, type, scale, true);
	// Remez rm(BinBoot, inter_num, inter_start, inter_end, sc, prec, deg, iteration, type, scale, true);
...
//	cout << "coefficients: " << endl;
	ofstream out1("../result/BinBootPower.txt"), out2("../result/BinBootcheb.txt");
	...
	// {
	// 	in >> num;
	// 	chebcoeff.push_back(num);
	// }
	// for (size_t i=0;i<chebcoeff.size();i++) {
	// 	RR num;
	// 	in >> num;
	// 	chebcoeff[i] = num;
	// }
	...
	// for (auto num : coeff) cout << num <<endl;

	// for(auto num : chebcoeff) cout << num << endl;
	// cout << endl;
	

	cout <<"input: "<<0.5001<<"BinBoot: "<<BinBoot( RR(-0.001) )<<", evaluate: "<<poly.evaluate_cheb( RR(-0.001) )<<endl;
	// test for coefficients
	RR max = RR(0);
	// for(RR x = RR(0); x<RR(351); x+=RR(1) ) if(abs(poly.evaluate(x/256)*256 - BinBoot(x/256)*256) > max) max = abs(poly.evaluate(x/256)*256 - BinBoot(x/256)*256 );
	// cout << "power basis approx maxerr: " << max << endl;
	max = RR(0);
	for(RR x = RR(0); x<RR(0.001); x+=RR(0.0001)) if(abs(poly.evaluate_cheb(x) - BinBoot(x)) > max) max = abs(poly.evaluate_cheb(x) - BinBoot(x));
