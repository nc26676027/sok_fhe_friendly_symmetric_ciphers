...
namespace boot
{
	...
	//	cout << endl;
		...
		for(long j = boundary_K - 1; j >= 0; j--) {
			for(long k = 0; k < nodecount[j]; k++) {
				if(j == 0) sample_point[ind].x = (-1) * width + width / (nodecount[j] + 1) * (k + 1);
				else sample_point[ind].x = (-1)*j - width + 2 * width / (nodecount[j] + 1) * (k + 1);
			...
			if(fracpart(scan_2) > width + sc / 2) {
				...
				while(!prec_end) {
					...
					if(tmpinc * prec_ext >= current_err) {
						...
						if(inc_2 == 1) local_extreme_point[local_extreme_count].locmm = 1;
						else local_extreme_point[local_extreme_count].locmm = -1;
						// cout << extreme_point[extreme_count].x << " " << extreme_point[extreme_count].y << " " << extreme_point[extreme_count].locmm << endl;
						local_extreme_count++;
						// cout << "Niu zi prec_end : " << prec_end << endl;
					}
				...
				if((inc_1 == 1 && inc_2 != 1) || (inc_1 == -1 && inc_2 != -1) || inc_1 == 0) {
					...
					while(!prec_end) {
						...
						while(prec_iter < params.binary_prec) {
							...
							// cout << "prec_ind : " << prec_ind << " prec_x : " << prec_x << endl;
							for(long k = 0; k < 3; k++) {
								if(inc_1 == 0) detail[k] = max(prec_x - prec_sc, scan_1) + prec_sc * k;
								else detail[k] = prec_x - prec_sc + prec_sc * k;
							}
							prec_iter++;
						}
						if(inc_2 == 1) tmpinc = -1;
	...
	void Remez::getextreme() {
		...
		extreme_count = 0;
		for (int i = 0; i < 2 * boundary_K - 1; i++) {
			// cout <<"local_extreme_count_array[i] = "<<local_extreme_count_array[i]<<endl;
	...
	void Remez::choosemaxs() {
		...
		// for (long i = 0; i < (deg + 2); i++){
		// 	cout <<"extract["<<i<<"] = "<<extract[i].x<<endl;
		// }
		// cout <<"before determine sample"<<endl;
		for(long i = 0; i < (deg + 2); i++) {
			// cout <<"iterate: "<<i<<endl;
			sample_point[i].x = extract[i].x;
			// cout <<"x: "<<sample_point[i].x <<endl;
			sample_point[i].y = function_value(sample_point[i].x);
			// cout <<"y: "<<sample_point[i].y <<endl;
		...
		// cout <<"after determine sample"<<endl;
	...
	void Remez::generate_optimal_poly(Polynomial &poly) {
		...
		// initialize();
		long it = 0;
		// cout <<" After better initial"<<endl;
		while((max_err - min_err) / min_err > approx) {
			// cout <<"before getcoeffwitherr"<<endl;
			getcoeffwitherr();
			// cout <<"getcoeffwitherr"<<endl;
			getextreme();
			// cout <<"getextreme"<<endl;
			choosemaxs();
			// cout <<"choosemaxs"<<endl;
			it++;
			// cout << "it: "<<it<<endl;
			// for(long i = 0; i < deg + 2; i++) {
		...
		// showcoeff();
		// ifstream in("../result/BinBootPower.txt");
		// for(int i = 0; i < deg + 1; i++) {
		// 	RR tmp;
		// 	in >> tmp;
		// 	coeff[i] = tmp;
		// }
		// poly.set_zero_polynomial(59);
		poly.set_polynomial(deg, coeff, "cheb");
		// poly.showcoeff();
		//	cout << "coefficients: " << endl;
		// RR x = RR(-0.501);
		// cout << "evaluate "<<x<<": "<<poly.evaluate(x)<<endl;
