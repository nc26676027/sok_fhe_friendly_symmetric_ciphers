...
type CKKSEvaluator interface {
	...
	ShallowCopy() CKKSEvaluator
	WithKey(EvaluationKey) CKKSEvaluator
}
...
type ckksEvaluator struct {
	*ckksEvaluatorBase
	*ckksEvaluatorBuffers
...
type ckksEvaluatorBuffers struct {
...
func newCKKSEvaluatorBase(params *Parameters) *ckksEvaluatorBase {
	var err error
	ev := new(ckksEvaluatorBase)
...
func newCKKSEvaluatorBuffers(evalBase *ckksEvaluatorBase) *ckksEvaluatorBuffers {
	buff := new(ckksEvaluatorBuffers)
	...
	buff.ctxpool = NewCiphertextCKKS(evalBase.params, 2, evalBase.params.MaxLevel(), evalBase.params.scale)
	return buff
}

// NewCKKSEvaluator creates a new Evaluator, that can be used to do homomorphic
// operations on the Ciphertexts and/or Plaintexts. It stores a small pool of polynomials
// and Ciphertexts that will be used for intermediate values.
func NewCKKSEvaluator(params *Parameters, evaluationKey EvaluationKey) CKKSEvaluator {
	eval := new(ckksEvaluator)
	eval.ckksEvaluatorBase = newCKKSEvaluatorBase(params)
	eval.ckksEvaluatorBuffers = newCKKSEvaluatorBuffers(eval.ckksEvaluatorBase)
...
func (eval *ckksEvaluator) permuteNTTIndexesForKey(rtks *RotationKeySet) *map[uint64][]uint64 {
	if rtks == nil {
...
func (eval *ckksEvaluator) ShallowCopy() CKKSEvaluator {
	return &ckksEvaluator{
		ckksEvaluatorBase:    eval.ckksEvaluatorBase,
		ckksEvaluatorBuffers: newCKKSEvaluatorBuffers(eval.ckksEvaluatorBase),
		rlk:                  eval.rlk,
		rtks:                 eval.rtks,
...
func (eval *ckksEvaluator) WithKey(evaluationKey EvaluationKey) CKKSEvaluator {
	...
	return &ckksEvaluator{
		ckksEvaluatorBase:    eval.ckksEvaluatorBase,
		ckksEvaluatorBuffers: eval.ckksEvaluatorBuffers,
...
func (eval *ckksEvaluator) getElemAndCheckBinary(op0, op1, opOut Operand, opOutMinDegree int) (el0, el1, elOut *Element) {
...
func (eval *ckksEvaluator) newCiphertextBinary(op0, op1 Operand) (ctOut *Ciphertext) {
	...
	return NewCiphertextCKKS(eval.params, maxDegree, minLevel, maxScale)
}

// Add adds op0 to op1 and returns the result in ctOut.
func (eval *ckksEvaluator) Add(op0, op1 Operand, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) AddNoMod(op0, op1 Operand, ctOut *Ciphertext) {
	el0, el1, elOut := eval.getElemAndCheckBinary(op0, op1, ctOut, utils.MaxInt(op0.Degree(), op1.Degree()))
...
func (eval *ckksEvaluator) AddNew(op0, op1 Operand) (ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) AddNoModNew(op0, op1 Operand) (ctOut *Ciphertext) {
	ctOut = eval.newCiphertextBinary(op0, op1)
...
func (eval *ckksEvaluator) Sub(op0, op1 Operand, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) SubNoMod(op0, op1 Operand, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) SubNew(op0, op1 Operand) (ctOut *Ciphertext) {
	ctOut = eval.newCiphertextBinary(op0, op1)
...
func (eval *ckksEvaluator) SubNoModNew(op0, op1 Operand) (ctOut *Ciphertext) {
	ctOut = eval.newCiphertextBinary(op0, op1)
	eval.SubNoMod(op0, op1, ctOut)
	return
}

func (eval *ckksEvaluator) evaluateInPlace(c0, c1, ctOut *Element, evaluate func(int, *ring.Poly, *ring.Poly, *ring.Poly)) {
...
func (eval *ckksEvaluator) Neg(ct0 *Ciphertext, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) NegNew(ct0 *Ciphertext) (ctOut *Ciphertext) {
	ctOut = NewCiphertextCKKS(eval.params, ct0.Degree(), ct0.Level(), ct0.Scale())
	eval.Neg(ct0, ctOut)
	return
}
...
func (eval *ckksEvaluator) AddConstNew(ct0 *Ciphertext, constant interface{}) (ctOut *Ciphertext) {
	ctOut = ct0.CopyNew().Ciphertext()
	eval.AddConst(ct0, constant, ctOut)
	return ctOut
}

func (eval *ckksEvaluator) getConstAndScale(level int, constant interface{}) (cReal, cImag, scale float64) {
...
func (eval *ckksEvaluator) AddConst(ct0 *Ciphertext, constant interface{}, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) MultByConstAndAdd(ct0 *Ciphertext, constant interface{}, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) MultByConstNew(ct0 *Ciphertext, constant interface{}) (ctOut *Ciphertext) {
	ctOut = NewCiphertextCKKS(eval.params, ct0.Degree(), ct0.Level(), ct0.Scale())
...
// The scale of the output element will depend on the scale of the input element and the constant (if the constant
// needs to be scaled (its rational part is not zero)). The constant can be a uint64, int64, float64 or complex128.
func (eval *ckksEvaluator) MultByConst(ct0 *Ciphertext, constant interface{}, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) MultByGaussianInteger(ct0 *Ciphertext, cReal, cImag int64, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) MultByGaussianIntegerAndAdd(ct0 *Ciphertext, cReal, cImag int64, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) MultByiNew(ct0 *Ciphertext) (ctOut *Ciphertext) {
	ctOut = NewCiphertextCKKS(eval.params, 1, ct0.Level(), ct0.Scale())
...
// It does not change the scale.
func (eval *ckksEvaluator) MultByi(ct0 *Ciphertext, ctOut *Ciphertext) {
...
// It does not change the scale.
func (eval *ckksEvaluator) DivByiNew(ct0 *Ciphertext) (ctOut *Ciphertext) {
	ctOut = NewCiphertextCKKS(eval.params, 1, ct0.Level(), ct0.Scale())
...
// It does not change the scale.
func (eval *ckksEvaluator) DivByi(ct0 *Ciphertext, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) ScaleUpNew(ct0 *Ciphertext, scale float64) (ctOut *Ciphertext) {
	ctOut = NewCiphertextCKKS(eval.params, ct0.Degree(), ct0.Level(), ct0.Scale())
...
func (eval *ckksEvaluator) ScaleUp(ct0 *Ciphertext, scale float64, ctOut *Ciphertext) {
	eval.MultByConst(ct0, uint64(scale), ctOut)
	ctOut.SetScale(ct0.Scale() * scale)
}

// SetScale sets the scale of the ciphertext to the input scale (consumes a level)
func (eval *ckksEvaluator) SetScale(ct *Ciphertext, scale float64) {
...
func (eval *ckksEvaluator) MulByPow2New(ct0 *Ciphertext, pow2 int) (ctOut *Ciphertext) {
	ctOut = NewCiphertextCKKS(eval.params, ct0.Degree(), ct0.Level(), ct0.Scale())
...
func (eval *ckksEvaluator) MulByPow2(ct0 *Element, pow2 int, ctOut *Element) {
	var level = utils.MinInt(ct0.Level(), ctOut.Level())
	ctOut.SetScale(ct0.Scale())
...
func (eval *ckksEvaluator) ReduceNew(ct0 *Ciphertext) (ctOut *Ciphertext) {
...
// To be used in conjunction with functions that do not apply modular reduction.
func (eval *ckksEvaluator) Reduce(ct0 *Ciphertext, ctOut *Ciphertext) error {
...
func (eval *ckksEvaluator) DropLevelNew(ct0 *Ciphertext, levels int) (ctOut *Ciphertext) {
	ctOut = ct0.CopyNew().Ciphertext()
...
// No rescaling is applied during this procedure.
func (eval *ckksEvaluator) DropLevel(ct0 *Ciphertext, levels int) {
	level := ct0.Level()
	for i := range ct0.value {
...
func (eval *ckksEvaluator) RescaleNew(ct0 *Ciphertext, threshold float64) (ctOut *Ciphertext, err error) {
...
func (eval *ckksEvaluator) Rescale(ctIn *Ciphertext, minScale float64, ctOut *Ciphertext) (err error) {
...
func (eval *ckksEvaluator) MulNew(op0, op1 Operand) (ctOut *Ciphertext) {
	ctOut = NewCiphertextCKKS(eval.params, op0.Degree()+op1.Degree(), utils.MinInt(op0.Level(), op1.Level()), 0)
...
func (eval *ckksEvaluator) Mul(op0, op1 Operand, ctOut *Ciphertext) {
	eval.mulRelin(op0, op1, false, ctOut)
}
...
func (eval *ckksEvaluator) MulRelinNew(op0, op1 Operand) (ctOut *Ciphertext) {
	ctOut = NewCiphertextCKKS(eval.params, 1, utils.MinInt(op0.Level(), op1.Level()), 0)
...
// The procedure will panic if ctOut.Degree != op0.Degree + op1.Degree.
// The procedure will panic if the evaluator was not created with an relinearization key.
func (eval *ckksEvaluator) MulRelin(op0, op1 Operand, ctOut *Ciphertext) {
	eval.mulRelin(op0, op1, true, ctOut)
}

func (eval *ckksEvaluator) mulRelin(op0, op1 Operand, relin bool, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) RelinearizeNew(ct0 *Ciphertext) (ctOut *Ciphertext) {
	ctOut = NewCiphertextCKKS(eval.params, 1, ct0.Level(), ct0.Scale())
...
func (eval *ckksEvaluator) Relinearize(ct0 *Ciphertext, ctOut *Ciphertext) {
	if ct0.Degree() != 2 {
...
func (eval *ckksEvaluator) SwitchKeysNew(ct0 *Ciphertext, switchingKey *SwitchingKey) (ctOut *Ciphertext) {
	ctOut = NewCiphertextCKKS(eval.params, ct0.Degree(), ct0.Level(), ct0.Scale())
...
// It requires a SwitchingKey, which is computed from the key under which the Ciphertext is currently encrypted,
// and the key under which the Ciphertext will be re-encrypted.
func (eval *ckksEvaluator) SwitchKeys(ct0 *Ciphertext, switchingKey *SwitchingKey, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) RotateNew(ct0 *Ciphertext, k int) (ctOut *Ciphertext) {
	ctOut = NewCiphertextCKKS(eval.params, ct0.Degree(), ct0.Level(), ct0.Scale())
...
// If the provided element is a Ciphertext, a key-switching operation is necessary and a rotation key for the specific rotation needs to be provided.
func (eval *ckksEvaluator) Rotate(ct0 *Ciphertext, k int, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) ConjugateNew(ct0 *Ciphertext) (ctOut *Ciphertext) {
	ctOut = NewCiphertextCKKS(eval.params, ct0.Degree(), ct0.Level(), ct0.Scale())
...
func (eval *ckksEvaluator) Conjugate(ct0 *Ciphertext, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) permuteNTT(ct0 *Ciphertext, galEl uint64, ctOut *Ciphertext) {
...
func (eval *ckksEvaluator) rotateHoistedNoModDown(ct0 *Ciphertext, rotations []int, c2QiQDecomp, c2QiPDecomp []*ring.Poly) (cOutQ, cOutP map[int][2]*ring.Poly) {
...
func (eval *ckksEvaluator) permuteNTTHoistedNoModDown(level int, c2QiQDecomp, c2QiPDecomp []*ring.Poly, k int, ct0OutQ, ct1OutQ, ct0OutP, ct1OutP *ring.Poly) {
...
func (eval *ckksEvaluator) SwitchKeysInPlaceNoModDown(level int, cx *ring.Poly, evakey *rlwe.SwitchingKey, pool2Q, pool2P, pool3Q, pool3P *ring.Poly) {
...
func (eval *ckksEvaluator) SwitchKeysInPlace(level int, cx *ring.Poly, evakey *rlwe.SwitchingKey, p0, p1 *ring.Poly) {
...
func (eval *ckksEvaluator) DecompInternal(levelQ int, c2NTT *ring.Poly, c2QiQDecomp, c2QiPDecomp []*ring.Poly) {
...
func (eval *ckksEvaluator) decomposeAndSplitNTT(level, beta int, c2NTT, c2InvNTT, c2QiQ, c2QiP *ring.Poly) {
...
func (eval *ckksEvaluator) permuteNTTHoisted(level int, c0, c1 *ring.Poly, c2QiQDecomp, c2QiPDecomp []*ring.Poly, k int, cOut0, cOut1 *ring.Poly) {
...
func (eval *ckksEvaluator) keyswitchHoisted(level int, c2QiQDecomp, c2QiPDecomp []*ring.Poly, evakey *rlwe.SwitchingKey, pool2Q, pool3Q, pool2P, pool3P *ring.Poly) {
...
func (eval *ckksEvaluator) keyswitchHoistedNoModDown(level int, c2QiQDecomp, c2QiPDecomp []*ring.Poly, evakey *rlwe.SwitchingKey, pool2Q, pool3Q, pool2P, pool3P *ring.Poly) {
