...
func (btp *Bootstrapper) subSum(ct *Ciphertext) *Ciphertext {
	...
	for i := btp.params.logSlots; i < btp.params.MaxLogSlots(); i++ {
		...
		btp.ckksEvaluator.Rotate(ct, 1<<i, btp.ckksEvaluator.ctxpool)

		btp.ckksEvaluator.Add(ct, btp.ckksEvaluator.ctxpool, ct)
	}
...
func (btp *Bootstrapper) evaluateSine(ct0, ct1 *Ciphertext) (*Ciphertext, *Ciphertext) {
	...
	btp.ckksEvaluator.scale = btp.sinescale // Reference scale is changed to the Qi used for the SineEval (which is also close to the new ciphetext scale)
...
func (btp *Bootstrapper) evaluateCheby(ct *Ciphertext) *Ciphertext {
	...
	for i := 0; i < btp.SinRescal; i++ {
		...
		if err := btp.Rescale(ct, btp.ckksEvaluator.scale, ct); err != nil {
			panic(err)
		}
	}
