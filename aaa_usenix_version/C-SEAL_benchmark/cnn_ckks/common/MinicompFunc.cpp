...
namespace minicomp
{
	...
	RR sgn(RR x) {
		if(x > 0) return static_cast<RR>(1);
		else if(x < 0) return static_cast<RR>(-1);
		else return static_cast<RR>(0);
	}
	RR comp(RR a, RR b) {
		if(a>b) return RR(1);
		else if(a<b) return RR(0);
		else return RR(0.5);
	}
	RR ReLU(RR x) 
	{
	...
	{
		//(1 - cos( pi * x)) / 2
		return ( 1 - cos( 2 * ComputePi_RR() * x ) ) / 2 ;
	}
	RR fracpart(RR x) {
		return x - round(x);
	}
	...
	RR expmaxerr(long deg, RR expx) {
		...
		for(long i=1; i<num; i++) {
			if(expx <= X[i]) {
				...
				if(X[i-1]<0 && X[i]>0) expy = Y[i];
				else if(Y[i-1]<0 && Y[i]>0) expy = Y[i];

				// normal case
				else	expy = Y[i-1] + (Y[i]-Y[i-1]) * (expx - X[i-1]) / (X[i] - X[i-1]);
				break;
			}
		}
		delete [] X;
	...
	//	RR K = RR(1000);
		...
		if(ext[ext_count].y > 0 ) ext[ext_count].locmm = 1;
		else ext[ext_count].locmm = -1;
		ext_count += 1;
		...
		if(scan_y1 < scan_y2) inc_2 = 1;
		else if(scan_y1 > scan_y2) inc_2 = -1;
	...
	void decrypt_and_print_part(const Ciphertext &cipher, Decryptor &decryptor, CKKSEncoder &encoder, long sparse_slots, size_t start, size_t end) {
		Plaintext plain;
		...
		cout << "( "; 
		cout << "... ";
		for (size_t i = start; i <= end; i++) cout << rtn_vec[i] << ", ";
		cout << "... ";
		cout << ")" << endl;
		cout << "scale: " << cipher.scale() << endl;
	}
	// print both cout and ofstream
	void decrypt_and_print_txt(const Ciphertext &cipher, Decryptor &decryptor, CKKSEncoder &encoder, long sparse_slots, size_t front, size_t back, ofstream &output) 
	{
		Plaintext plain;
		decryptor.decrypt(cipher, plain);
		...
		{
			cout << rtn_vec[i] << ", ";
			if((i+1)%1024 == 0) cout << endl << endl;
		}
		for (size_t i = 0; i < front; i++) 
		{
			output << rtn_vec[i] << ", ";
			if((i+1)%1024 == 0) output << endl << endl;
		}
		cout << "... ";
		output << "... ";
		...
		// double max_abs = 0;
		// for(size_t i=0; i<n; i++) if(max_abs < abs(rtn_vec[i])) max_abs = abs(rtn_vec[i]);
		// cout << "max abs: " << max_abs << endl;
		// output << "max abs: " << max_abs << endl;
	}
}
